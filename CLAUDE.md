...
- **Keep docs in sync.** Whenever a new diagram type lands, rendering/engine behavior changes, authoring syntax changes (elements, attributes, styles, defaults), or anything else user-facing changes, update every affected surface in the same change — don't defer it:
  ...
  - `README.md` (the **Examples** section — one render per type, add/refresh whenever a diagram type is added or changed — plus quickstart and doc links). Examples are pure Markdown (no raw HTML, so they render in editors like Zed too): each points its image at the case's committed `tests/cases/<type>-basic/golden.png` and its **source** link at the sibling `ours.html`, so image and source share a folder and there are no derived assets to keep in sync — the golden re-blesses with the suite. (In pure Markdown an image's on-screen size equals its pixel size capped at page width; wide diagrams like deployment are therefore height-limited by that cap.)
