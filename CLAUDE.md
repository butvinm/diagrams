...
- **Keep docs in sync.** Whenever a new diagram type lands, rendering/engine behavior changes, authoring syntax changes (elements, attributes, styles, defaults), or anything else user-facing changes, update every affected surface in the same change — don't defer it:
  ...
  - `README.md` (the **Examples** section — one render per type, add/refresh whenever a diagram type is added or changed — plus quickstart and doc links). Examples use pure Markdown (no raw HTML, so they render in editors like Zed too) pointing at `docs/examples/*.png` thumbnails, regenerated from the goldens via `for t in sequence state class deployment; do magick tests/cases/$t-basic/golden.png -resize x240 docs/examples/$t.png; done` — re-run when a golden changes.
