...
Labels and boxes can also hold **LaTeX math**: KaTeX (vendored, so it works offline) typesets `\(`…`\)` inline and `\[`…`\]` / `$$`…`$$` display — e.g. `<arrow from="a" to="b">\(g^{ab} \bmod p\)</arrow>`. See the [component reference](../diagrams/skills/diagrams/references/COMPONENTS.md#math-latex-via-katex).

## Embedding in a page

The default output is a PNG, but a diagram can also ship as **live HTML**. `render.mjs --html` emits a self-contained file (kit, and KaTeX when needed, inlined) that you embed with an `<iframe>` — text stays selectable and it's crisp at any zoom. Add the `fluid` modifier (`<diagram class="… fluid">`) and the diagram fills its container, so a CSS-responsive layout can flex its gutters (`space-between`) and reflow at `@media` breakpoints; the kit redraws the connectors as it resizes. Because connectors never route, responsiveness is _size-adaptive_, not _topology-adaptive_ — clean when spacing/scale change, but a reflow that changes which boxes are adjacent will cross fixed anchors. See [`EMBED.md`](../diagrams/skills/diagrams/references/EMBED.md) and the [live gallery](https://butvinm.github.io/diagrams/).

## Diagram types

Every diagram type is the same boxes-and-arrows underneath; a type just adds a layout convention and a few ready-made box styles. Sequence diagrams use a grid preset (with `<activation>` bars, `<fragment>`/`<guard>` combined fragments like `alt`/`opt`/`loop`, and `<divider>` section bands), state machines a centered `stack`, class diagrams use `.class` boxes (with a `hollow` arrowhead for UML generalization), and deployment diagrams use `.node` containers holding nested `.artifact` boxes — both placed on your own CSS layout. Data flow diagrams add `.external` / `.process` / `.store` elements, plus `.zone` and `.boundary` for the threat-model flavor (trust zones and boundary lines). Flowcharts add the process-flow node shapes `.terminal` / `.step` / `.decision` / `.io` / `.subroutine` / `.database` / `.preparation`. Pick the matching type reference below.

## Reference

The core components, attributes, and styles: [`COMPONENTS.md`](../diagrams/skills/diagrams/references/COMPONENTS.md). Then the per-type files — [`SEQUENCE.md`](../diagrams/skills/diagrams/references/SEQUENCE.md), [`STATE.md`](../diagrams/skills/diagrams/references/STATE.md), [`CLASS.md`](../diagrams/skills/diagrams/references/CLASS.md), [`DEPLOYMENT.md`](../diagrams/skills/diagrams/references/DEPLOYMENT.md), [`DFD.md`](../diagrams/skills/diagrams/references/DFD.md), [`FLOWCHART.md`](../diagrams/skills/diagrams/references/FLOWCHART.md). For embedding in web pages, [`EMBED.md`](../diagrams/skills/diagrams/references/EMBED.md).
